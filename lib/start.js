...
async function start() {
  ...
  let trumpSuitText = (0, _generators.generateTrumpSuitTextSprite)(game.trumpCard || game.trumpSuit);
  ...
  (0, _eventHandlers.onGameUpdate)(newGameObj => {
    ...
    _app.app.stage.addChild(deckCountText); // Update trump suit text if trump has been declared
  ...
  (0, _eventHandlers.onTrumpDeclared)(trumpDeclaration => {
    // Remove trump declaration buttons when trump is declared
    removeTrumpDeclarationButtons(); // Add trump suit text if it doesn't exist yet

    if (!trumpSuitText && trumpDeclaration.suit) {
      trumpSuitText = (0, _generators.generateTrumpSuitTextSprite)(trumpDeclaration.suit);
  ...
  (0, _eventHandlers.onTrumpDeclarationRejected)(reason => {
  ...
  setUpOpponentBackOfCards(opponentBackOfCardSprites); // Initialize trump declaration buttons if needed
  ...
  function createTrumpDeclarationButtons(availableDeclarations) {
    ...
    availableDeclarations.forEach((suit, index) => {
      ...
      buttonText.on('pointerdown', () => {
        (0, _eventHandlers.declareTrump)(suit);
      });
      trumpDeclarationButtons.push(buttonText);

      _app.app.stage.addChild(buttonText);
    });
  }
  ...
  function updateTrumpDeclarationButtons(game) {
    if (game.gameVariant === _Constants.Constants.gameVariants.BRISCOLA_500 && !game.trumpDeclared) {
      ...
      if (isMyTurn) {
        // Get player index
        const playerIndex = game.players[0].socketId === myPlayerObject.socketId ? 0 : 1; // Create a proper Game instance to access methods

        const gameInstance = new _Game.Game(game); // Get available trump declarations for current player
