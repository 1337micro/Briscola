...
class Game {
  constructor(gameState = {}) {
    ...
    this.trumpCard = gameState.trumpCard ? new _Card.Card(gameState.trumpCard) : null;
  ...
  init() {
    ...
    this.players = [this.player1, this.player2]; // For traditional variant, draw trump card. For Briscola 500, no initial trump
  ...
  computerAI(computerHand) {
    const trumpCard = this.middlePile.trumpCard; //player hand sorted ascending in point value
  ...
  canDeclareTrump(playerIndex, suit) {
    ...
    const playerCards = player.hand.cards; // Check if player has both king and horse of the specified suit
  ...
  declareTrump(playerIndex, suit) {
    ...
    this.trumpSuit = suit;
    this.trumpDeclared = true; // Update middle pile with the new trump suit
